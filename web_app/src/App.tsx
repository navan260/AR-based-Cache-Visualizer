...
function App() {
  // Configuration State
  const [memorySizeKB, setMemorySizeKB] = useState(16);
  const [cacheSizeKB, setCacheSizeKB] = useState(1);
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 bg-white/5 backdrop-blur-lg rounded-2xl p-6 border border-white/10 shadow-xl">
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-sm text-slate-300 mb-1">Memory Size (KB)</label>
              <select value={memorySizeKB} onChange={e => setMemorySizeKB(Number(e.target.value))}
                className="w-full bg-slate-800 border border-slate-600 rounded-lg p-2 focus:ring-2 focus:ring-cyan-500 outline-none">
                {[4, 8, 16, 32, 64].map(v => <option key={v} value={v}>{v} KB</option>)}
              </select>
            </div>

            <div>
              <label className="block text-sm text-slate-300 mb-1">Cache Size (KB)</label>
              <select value={cacheSizeKB} onChange={e => setCacheSizeKB(Number(e.target.value))}
                className="w-full bg-slate-800 border border-slate-600 rounded-lg p-2 focus:ring-2 focus:ring-cyan-500 outline-none">
                {[0.5, 1, 2, 4].map(v => <option key={v} value={v}>{v} KB</option>)}
              </select>
            </div>

            <div>
              <label className="block text-sm text-slate-300 mb-1">Block Size (Bytes)</label>
              <select value={blockSizeBytes} onChange={e => setBlockSizeBytes(Number(e.target.value))}
                className="w-full bg-slate-800 border border-slate-600 rounded-lg p-2 focus:ring-2 focus:ring-cyan-500 outline-none">
                {[8, 16, 32, 64].map(v => <option key={v} value={v}>{v} Bytes</option>)}
              </select>
            </div>
            ...
            <button onClick={initializeCache}
